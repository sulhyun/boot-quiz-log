...
public class MailController {
	...
	@ResponseBody
	@PostMapping("/setCode")
	public boolean setCode(@RequestBody Map<String, String> map) {
		System.out.println("이메일 인증 코드 전송");
		String mb_email = map.get("mb_email");
		String code = customUtil.getCustomNumber(6);
		String mail = "안녕하세요.<br>퀴즈QNL 회원 인증 메일입니다.<br>6자리 코드를 회원가입 창에서 입력해주세요. <p>인증코드 : <strong>" + code + "</strong></p>";
		boolean res = mailService.setMailCode(mb_email, code);
		if(res) {
			mailSend(mb_email, "퀴즈QNL 인증 이메일", mail);
		}
		return res;
	}

	private boolean mailSend(String to, String title, String content) {
	    String from = "quizQNL@gmail.com";
	    try{
	...
	@ResponseBody
	@PostMapping("/getCode")
	public boolean getCode(@RequestBody Map<String, String> map) {
		System.out.println("이메일 인증 코드 확인");
		String mb_email = map.get("mb_email");
	...
	@ResponseBody
	@PostMapping("/delCode")
	public boolean delCode(@RequestBody Map<String, String> map) {
		System.out.println("이메일 인증 코드 삭제");
		String mb_email = map.get("mb_email");
